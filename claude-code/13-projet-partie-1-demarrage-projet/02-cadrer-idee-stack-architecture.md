...
leçon: 02-cadrer-idee-stack-architecture
statut: à revoir
etape_revision: 0
prochaine_revision:
---
...
Conséquence : L'agent invente une architecture, choisit une stack arbitraire, et commence à créer des fichiers.
...
```text
             SESSION DE CADRAGE (PROMPT ARCHITECT)
                           │
           ┌───────────────┼───────────────┐
           ↓               ↓               ↓
      1. RÔLE & RÈGLES  2. INTERVIEW    3. LIVRABLE UNIQUE
      (Pas de code !    (L'agent vous   (Le design-doc.md
      Ne pas inventer)  pose des Qs)     écrit sur disque)
           │               │               │
           └───────────────┬───────────────┘
                           ↓
                  RÉSULTAT SÉCURISÉ :
         Le périmètre est fermé, la stack validée,
        et les "Non-objectifs" empêchent le hors-sujet.
```
